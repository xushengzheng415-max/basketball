...
function buildFullContent() {
  return `
<p style="text-indent:2em;">2026年中国小篮球系列活动全国总决赛正在北京延庆进行。这届赛事除了等级门槛引发讨论，还有一件事正在发生：中国篮球协会通过"中国篮球APP"完成报名、身份核验和数据采集，并开始建立全国6至12岁小球员的数据档案。</p>

<p style="text-indent:2em;">对基层篮球培训机构来说，这条信息很容易被忽略。毕竟，机构负责人更关心的是孩子能不能参赛、比赛打得怎么样。但篮协这一步释放的信号很清楚：青少年篮球正在从"办比赛"走向"建数据"。</p>

<p style="text-indent:2em;">问题是，基层机构跟得上吗？</p>

<p style="text-align:center;margin:16px 0;"><img src="${MAIN_IMAGE_URL}" alt="教练在球场边记录训练数据" style="max-width:100%;height:auto;" /></p>
<p style="text-align:center;color:#888;font-size:12px;">数据化管理的起点可以很低，一张纸、一支笔就能开始｜来源：AI生成</p>
...
<div style="margin:24px 0;padding:16px;background:#fafafa;border-radius:8px;">
<p style="text-indent:0;text-align:center;font-size:15px;color:#333;margin-bottom:12px;"><strong>加入赛小蜂篮球社群</strong></p>
<p style="text-indent:0;text-align:center;font-size:13px;color:#666;margin-bottom:12px;">与篮球培训机构从业者交流经营经验、赛事资讯和行业动态</p>
<p style="text-align:center;"><img src="${COMMUNITY_QR_URL}" alt="加入群聊" style="max-width:280px;width:70%;height:auto;" /></p>
<p style="text-indent:0;text-align:center;font-size:12px;color:#999;">长按识别二维码，加入社群</p>
</div>

<p style="text-align:center;margin:20px 0;"><a data-miniprogram-appid="wx06d735da15276acd" data-miniprogram-path="pages/home/index" href="https://mp.weixin.qq.com" style="display:inline-block;padding:10px 24px;background:#ff6b35;color:#fff;text-decoration:none;border-radius:4px;font-size:14px;">进入赛小蜂篮球首页</a></p>
...
async function updateDraft() {
  ...
  // 保存到本地备份
  const backupPath = 'E:/Documents/saixoafeng_basketball/公众号/8月推文/麦步赛事/8月6日/文字稿/完整排版.html';
  ...
  const payload = {
    ...
    index: 0,
    articles: {
      title: "小篮球总决赛推数字化档案，基层机构的数据\"跟还是不跟\"？",
      author: process.env.ARTICLE_AUTHOR || '麦步赛事',
      digest: "2026年全国小篮球总决赛引入数字化档案建设，基层篮球培训机构面临数据鸿沟。本文提出三段式跟进框架：纸笔记录、简单工具整理、平台接入，帮助机构按自身节奏推进数据化管理。",
      content: content,
      content_source_url: '',
  ...
  const result = await requestJson(
    `${WECHAT_BASE}/cgi-bin/draft/update?access_token=${encodeURIComponent(token)}`,
    {
      method: 'POST',
  ...
  console.log('完整排版已保存到：' + backupPath);
}
