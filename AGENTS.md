...
> 当前用户版本锁定规则：赛小蜂篮球当前版本固定为 2.1.0。在用户明确确认“审核上线成功”前，开发、预览、联调、体验测试和同功能修正均沿用 2.1.0，不得自行递增版本号，也不得创建新的公开版本公告。只有用户明确确认审核上线成功后，才允许确定并启用下一版本号。
...
- 用户已授权：针对其明确要求修复或实现的篮球项目功能，完成本地验证后可直接部署对应云函数或静态托管文件，无需再次询问是否部署；部署后必须回读状态。该授权不包含删除线上数据、正式发布小程序、提交审核、付费操作或 Git 远端推送，这些高风险动作仍需单独明确授权。
...
根据 README、开发记录与最新产品决策：小程序现有能力仍以赛事 MVP 为主；PC 端的主产品定位是“赛训经营增长平台”，赛事中心为随机构开户默认启用的免费模块。
...
| PC 免费模块 | 赛事中心 | 赛事、赛制、赛程、场次、现场人员安排、赛果和赛事数据；随机构开户默认启用 |
| 小程序现场工具 | 横屏计分台、球员数据统计台、MC 控制台 | 比赛现场执行与协同控制 |

教务经营、交易与财务属于付费能力，购买授权码后单独激活，不影响免费赛事中心使用。
...
- 平台账号登录统一使用微信身份，不再以密码作为主登录方式；PC 使用微信开放平台网站应用扫码登录，小程序使用微信授权登录。
- 首个创建或认领机构的微信平台账号默认为机构老板/主账号，对应机构成员角色 `owner`；同一平台账号在 PC 和小程序中均默认显示该机构老板身份和机构信息。
- 机构老板可为主账号绑定手机号；手机号用于安全校验、服务联系和异常恢复，不恢复密码登录主流程。
- 机构教练由机构老板或获授权管理员添加；支持微信扫码、邀请链接、服务号通知等方式加入。教练确认后新增机构成员关系，不新建重复平台账号。
- 教练的长期机构成员身份与单场比赛临时助教权限分开建模；加入机构不代表自动获得所有赛事、课程或经营数据权限。

- 计分板无论手机或 PAD，统一只提供横屏控制台；不再设计、开发或验收竖屏计分界面。
- 主教练与助教的手机横屏和PAD横屏必须使用同一信息架构、同一数据项、同一按钮集合和同一权限逻辑；只允许响应式排版、滚动、折叠或分页，禁止手机端删减功能。主裁判继续使用已经完成的既有控制台。
- 现场核心工作面固定为：主裁判控制台、主队主教练端、客队主教练端，三端围绕同一平台、同一赛事和同一场次协作。
- 现场系统启动流程保持简单：PC 确定主裁判 → 双方球队递交首发与替补名单 → 数据管理员确认三项条件并一键启动 → 各角色进入操作页。
- 主裁判端不配置球队名单、助理教练或统计口径，只核对双方球队、时间和场地后进入比分/时间/MC 控制台。
- 主裁判控制官方比分、比赛时间、节次、24/14 秒、球队犯规、暂停、球权、比赛结束和 MC；MC 不再作为独立第三席。
- 已完成的主裁判比赛控制台属于锁定基线，不重构页面、不改比分等现有数据结构。正式比赛禁用“显示阵容”和主裁判球员统计；机构内部比赛继续保留阵容与球员统计功能。
- 主裁判PAD与手机横屏均已有现成页面，两端都锁定不重做；基准素材分别为 `docs/原型-裁判教练协同/assets/主裁判原版控制台.jpg` 和 `主裁判手机横屏原版控制台.jpg`。
- 比赛类型由 PC 场次配置决定，主裁判不得在比赛中自行切换正式/内部模式；禁用阵容时只关闭入口和写权限，不删除底层阵容与统计能力。
- 原球员数据统计员任务按主客队拆为两份，分别分发给双方主教练。教练只记录本队阵容、换人、球员得分归属和个人技术统计，只读同步官方比分和比赛时间，不能修改官方赛果。
- 每队主教练是本队数据总负责人，可亲自记录、只使用战术板，或邀请最多 5 名助理教练协作；主教练不占 5 名助教上限。
- 主教练任务编排主流程只保留按数据分工和按球员分工。按数据分工时，教练负责全队的指定数据；按球员分工时，教练可记录指定球员的全部数据。自定义高级分工保留为次级入口。
- 分配对象只保留两种状态：绿色可分配、灰色已分配并显示负责人。先点教练，再点绿色数据或球员即直接分配并变灰，不设“选择中”状态和二次确认。
- 助教来源分为球队已有教练和临时助教。已有教练可直接加入本场；临时助教通过手机号、服务号、二维码、邀请链接或历史比赛邀请，确认后才能参与分工，权限只限本队本场。
- 平台不强制完整统计。未选择的指标必须显示“未统计”而不是 0；只有完整球员得分归属需要与裁判官方比分严格一致并进入裁判强一致核验。
- 助理教练只能操作本人任务范围并修正本人事件。所有权限必须在赛前分配；开赛后分工自动锁定，主教练只能查看进度并负责整队汇总和最终提交，不能赛中重新分配。
- 主教练在比赛前完成助教权限分配，可通过服务号通知、邀请链接或现场二维码发送；助教确认本人任务并绑定后直接操作，权限只限本队本场。
- 双方主教练可自主选择是否记录本队数据；主客队分别提交、分别由主裁判核验、分别发布，一方状态不得阻塞另一方或官方赛果。
- 教练使用平板时，数据端提供一键进入战术板的能力。战术内容属于球队私有，不进入官方事件流，不向裁判、对手或主办方公开。
- 战术板统一使用 `docs/原型-裁判教练协同/assets/球场版面.png` 球场底图；本方球员使用头像加小号球衣号码，对方球员使用高对比圆形加大号号码。
- 战术板在手机和PAD上都尽量全屏显示球场，路线、画笔、传球、防守、文字、橡皮、撤销、清空等工具悬浮在球场上，不使用占据大面积的左右工具栏。
- 教练自录数据即使经主裁判确认，也不得自动用于赛事 MVP、得分王、最佳阵容等官方个人奖项。
- 不建立跨机构通用裁判库或长期裁判角色入口。机构赛事管理员在 PC 为场次分配主裁判，内部人员或外请人员通过服务号接受指定场次任务。
- 外请主裁判不录入机构教练库，不进入课程、课消、薪酬或经营数据；其身份和权限仅限被分配场次。
- 最新协同原型与运行规则位于 `docs/原型-裁判教练协同/`；旧三席记录仅保留历史与迁移说明。
- 教练团队任务分发的完整规则位于 `docs/教练团队球员数据任务分发规则.md`。
---
