...
import shutil
root = Path(r'E:\Documents\sxf-basketball')
upload = Path(r'C:\Users\Frank\Documents\feishu-upload')
imgdir = upload / 'images'
imgdir.mkdir(parents=True, exist_ok=True)
items = [
('01-login.png', root/'docs/原型-v1-位图/01-登录页.png','登录页','1.0必做','微信登录、游客体验、协议勾选、品牌露出。'),
('02-home.png', root/'docs/原型-v1-位图/02-首页.png','首页','1.0必做','快速开始比赛、最近赛果、创建赛事、球员库入口。'),
('03-quick-match.png', root/'docs/原型-v1-位图/03-快速比赛设置.png','快速比赛设置','1.0必做','设置主客队、节数、每节时长、计时方式。'),
('04-scorer-mobile.png', root/'docs/原型素材/页面/33-scorer-mobile-final-drawers-small-mc.png','竖屏裁判比分板最终版','1.0必做','核心计分、计时、犯规、暂停、换人、技术统计、MC小按钮、左右球员抽屉、交换场地。'),
('05-scorer-pad.png', root/'docs/原型素材/页面/32-scorer-pad-final-no-referee-checkin.png','PAD横屏比分板最终版','1.0建议做','记录台/PAD横屏使用，三栏布局，左右首发/替补抽屉，中间交换场地，底部MC面板。'),
('06-mc-drawer.png', root/'docs/原型-v1-位图/05-MC音效抽屉.png','MC音效抽屉','1.0必做','现场欢呼、进攻、防守、暂停、得分播报等音效入口。'),
('07-mine.png', root/'docs/原型-v1-位图/06-我的页面.png','我的页面','1.0必做','账号信息、会员、反馈、音效设置入口。'),
('08-mc-settings.png', root/'docs/原型-v1-位图/07-MC音效设置.png','MC音效设置','1.0必做','音效开关、随机/固定播放、语音包、外设快捷键配置。'),
('09-tournament-list.png', root/'docs/原型-v1-位图/09-赛事列表.png','赛事列表','1.0必做','赛事列表、状态筛选、创建赛事入口。'),
('10-tournament-detail.png', root/'docs/原型-v1-位图/10-赛事详情场次.png','赛事详情/场次管理','1.0必做','赛事信息、添加场次、添加球队、球员名单、进入计分。'),
('11-player-library.png', root/'docs/原型-v1-位图/11-球员库.png','球员库','1.0必做','添加球员、姓名号码、所属球队、编辑球员。'),
('12-pc-admin.png', root/'docs/原型-v1-位图/08-PC管理后台.png','PC管理后台','1.0必做','用户、会员码、权益、MC音乐库管理。'),
]
