...
function buildEntitlement(code, wxContext, now) {
  ...
  const entitlement = {
    ...
    features: Array.isArray(code.features) ? code.features : ['mc_system', 'stats_scorer_2'],
    voiceCredits: Math.max(0, Math.floor(Number(code.voiceCredits || 0))),
    shareCredits: Math.max(0, Math.floor(Number(code.shareCredits || 0))),
    status: 'active',
    source: 'redeem_code',
    startedAt: now,
    createdAt: now,
